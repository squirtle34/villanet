	 ...
	 <form action="" method="post">
		...
		<tbody>
       	...
       	<tr>
			...
			<td>   <?php if($op=='' && $eli<>1 ) {?>
<select name="cliente"  class="inp-form" required style="width:200px;height:33px;font-size: 12px;">
  <option value="">Elija el Cliente</option>
<?php
$consulta=mysql_query("SELECT * FROM persona   ");
while($datos=mysql_fetch_array($consulta))
{  print '<option value="'.$datos[0].'">'.strtoupper($datos[1]).'</option>';}
...
?>
            ...
            </select><?php } else {  $sql=mysql_query("Select * from persona,ventas where rifc=cliente and n_control='$n_control'") or die("error"); 
			$nombre=mysql_fetch_array($sql);
				print $nombre[razon_social];  }?> 
...
<tr>
			...
			<td>
<select name="producto"  class="inp-form" onclick="ver('0')" required style="width:200px;height:33px;font-size: 12px;">
...
?>
				<tr>
					...
					<td>LA VILLA MAYOR</td><td><?php print $precio;?></td>
					...
					<a href="index.php?menu=1&url=7&idc=<?php print $precio;?>&idp=<?php print $datpro[0];?>&ncontrol=<?php print $n_control;?>&eli=1&id=<?php print $detalles_producto[7];?>"   title="Eliminar" class="icon-2 info-tooltip"></a> 
					</td>
				</tr> 
<?php }?>	
<tr>
...
<form action="?menu=1&url=7" method="post">
...
$sql=mysql_query("SELECT *FROM ventas where status='0'");

$num=mysql_num_rows($sql);
if($num >0)
{ while($ventas_clientes = mysql_fetch_array($sql))
{
print '<div class="right"><a href="index.php?menu=1&url=7&ncontrol='.$ventas_clientes[n_control].'&eli=1&id=0"><img src="images/forms/icon_plus.gif" alt="" height="21" width="21">&nbsp;&nbsp;Numero de Pedido:'.$ventas_clientes[n_control].'</a></div>';
